...
- 區網即時搜尋／下載進度：http://10.0.1.118:8770/
...
- 每小時把 10.0.1.118 進度寫入 [`docs/data/progress.json`](docs/data/progress.json) 並推到 GitHub Pages。
- 每個疾病累積 ≥ 4 筆完整 FASTQ 後，自動 `POST /api/analyze`
- 舊的 252 筆 rawdata 不會混進這次分析（只配對 SRA metadata）

在 http://10.0.1.118:8765/ 選 metadata 檔 `metadata_sra_CRC.tsv` 等即可看進度。
